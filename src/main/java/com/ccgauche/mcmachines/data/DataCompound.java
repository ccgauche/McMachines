...
public class DataCompound {
	...
	public void updateStack(@NotNull ItemStack stack) {
		...
		String id = DataRegistry.ID.get(this);
		if (id != null) {
			lore.add(TextUtils.from("§r§7" + id));
		}
		Integer maxEnergy = DataRegistry.ENERGY_MAX.get(this);
		if (maxEnergy != null) {
			lore.add(TextUtils.from(
					"§r§fEnergy§6: §e" + DataRegistry.ENERGY_CONTENT.getOrDefault(this, 0) + "§f/§e" + maxEnergy));
		}
	...
	public String toString() {
		return "DataCompound{" + "object=" + object + '}';
	}
}
